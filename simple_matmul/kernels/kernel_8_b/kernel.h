...
// Additional macros and functions for shared memory optimization (GPUGenie Step)
...
// New macros for coarsening (GPUGenie: Coarsen Y Step 1)
...
// New macros for coarsening (GPUGenie: Coarsen X Step 1)
...
__global__ void foo(const float* A, const float* B, float* C, int n) {
    ...
    // Declare accumulator array for coarsened output
    float accumulator_array[COARSEN_Y_VALUE * COARSEN_X_VALUE] = {0};

    // Declare register cache for B_shared
    ...
    for (int k_outer = 0; k_outer < n; k_outer += TUNE_SPLIT_VALUE) {
        ...
        for (int k_inner = 0; k_inner < TUNE_SPLIT_VALUE; k_inner++) {
            ...
            // Coarsen Y Accumulate Transformation
